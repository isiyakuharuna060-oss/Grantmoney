...
export default function App() {
  ...
  // Fetch client IP and Firebase status on load
  useEffect(() => {
    fetch("/api/ip")
      .then((res) => res.json())
      .then((data) => {
  ...
  // Fetch submitted applications for Admin database handling view with browser-level backup replication
  const fetchRecords = async () => {
    try {
      const res = await fetch("/api/grants");
      const serverData = await res.json();
      const serverRecords = Array.isArray(serverData) ? serverData : [];

      // Retrieve browser local backup copies
      let localBackups: any[] = [];
      try {
        const stored = localStorage.getItem("local_grants_backup");
        if (stored) {
          localBackups = JSON.parse(stored);
        }
      } catch (err) {
        console.error("Error reading browser backup copies:", err);
      }
      ...
      localBackups.forEach((localRec: any) => {
        if (!serverIds.has(localRec.id)) {
          merged.push({
            ...localRec,
            isBrowserBackup: true
          });
        }
      });
      ...
      // Failover to client backups so the dashboard isn't empty
      try {
        const stored = localStorage.getItem("local_grants_backup");
        if (stored) {
          const backups = JSON.parse(stored).map((r: any) => ({ ...r, isBrowserBackup: true }));
          setRecords(backups);
        }
      } catch (e) {
        console.error("Backup failover failed:", e);
      }
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Replicate a backup copy in local storage so it remains accessible if server reboots
        try {
          const backupRecord = {
          ...
          const existing = JSON.parse(localStorage.getItem("local_grants_backup") || "[]");
          existing.unshift(backupRecord);
          localStorage.setItem("local_grants_backup", JSON.stringify(existing));
        } catch (e) {
          console.error("Error replicating local backup:", e);
        }
        setStep(5);
      } else {
            ...
            !isAdminAuthenticated ? (
                ...
                <div className="flex items-center justify-between gap-4 mb-6">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100 transition-all flex items-center justify-center shadow-sm"
              ...
              <div className="flex-1 overflow-y-auto max-h-[360px] pr-1 space-y-3 scrollbar-thin">
                {filteredRecords.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-400 text-center px-4 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                    <AlertCircle className="w-8 h-8 mb-2 text-indigo-400" />
                    <p className="text-sm font-semibold text-slate-800">No applications found in the database</p>
                    <p className="text-[11px] text-slate-500 mt-1 max-w-sm leading-relaxed">
                      {firebaseStatus?.connected ? (
                        "Your Cloud Firestore is connected, but this database collection is currently empty. Go back to the registration page, submit a test application, and it will show up here instantly!"
                      ) : (
                        "The database is currently empty. If you just connected Firebase, submit a new test application now that Firebase is connected to start saving applications permanently to your cloud!"
                      )}
                    </p>
                  </div>
                ) : (
                      ...
                      <div className="flex justify-between items-start border-b border-slate-100 pb-2">
                        <div>
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {rec.isBrowserBackup && (
                              <span className="px-1.5 py-0.5 rounded bg-amber-50 border border-amber-200 text-amber-700 font-bold text-[9px] uppercase tracking-wider">
                                Device Backup
                              </span>
                            )}
                          </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {step === 5 && (
                    ...
                    >
                      ...
                      {/* Explicit feedback about connection status */}
                      <div className="max-w-xs mx-auto p-3.5 rounded-2xl bg-slate-50 border border-slate-200/60 text-[11px] text-slate-600 font-sans space-y-1.5 shadow-sm">
                        {firebaseStatus?.connected ? (
                          <>
                            ...
                            <p className="text-[10px] text-slate-500 leading-normal">
                              This application has been saved directly to your live <strong>Cloud Firestore</strong> database. You can instantly view it in the Admin Portal!
                            </p>
                          </>
                        ) : (
                          <>
                            ...
                            <p className="text-[10px] text-slate-500 leading-normal">
                              The application was successfully stored locally on the server. To sync submissions permanently, please connect Firebase via the Admin settings panel.
                            </p>
                          </>
                        )}
                      </div>
